...
CANDLE_GUIDE = """\
[봉(캔들) 읽는 법]
컨텍스트의 candles[종목] 는 봉 배열이고, 각 봉은 [날짜, 시가, 고가, 저가, 종가, 거래량] 이며 과거→현재 순이다.
(kospi_proxy.candles 도 같은 형식. candle_format 필드에 형식이 명시돼 있다.)
- 종가 > 시가 면 양봉(상승), 종가 < 시가 면 음봉(하락). 시가~종가 '몸통'이 길수록 그 방향 힘이 세다.
- 고가-몸통상단 = 윗꼬리: 길면 위에서 매물이 나와 상승이 눌린 것(저항). 몸통하단-저가 = 아랫꼬리: 길면 저점을 지킨 것(지지).
- 최근 봉들의 종가가 꾸준히 높아지면 상승추세, 낮아지면 하락추세. 거래량이 함께 늘면 그 움직임의 신뢰도가 높다.
- 거래량 없는 가격 변동이나 한두 개 봉의 급변(긴 꼬리/갭)은 신뢰하지 말고 추세 전체로 판단하라.
- 고점 부근의 긴 윗꼬리·대량 음봉은 매도 신호, 저점 부근의 긴 아랫꼬리·거래량 동반 양봉은 반등 신호로 가중한다."""
...
원칙:
- 봉(캔들)으로 추세·지지/저항·거래량을 먼저 읽고, 단순 현재가만 보고 판단하지 마라.
...
- screening_reasons 가 있으면 선정 사유(뉴스/시장 변화)를 참고하되 맹신하지 말고 시세/봉으로 교차검증하라.
반드시 submit_decision 도구로만 답한다. 종목마다 한 번씩 호출한다."""

REVIEW_SYSTEM = """\
너는 1차 엔진의 매매 제안을 검증하는 신중한 상위 검토자(게이트키퍼)다.
1차가 '사자/팔자'(BUY/SELL)고 한 제안만 받는다.

""" + CANDLE_GUIDE + """

각 제안에 대해:
- 봉(캔들)·추세·보유현황·리스크에 비추어 타당하면 APPROVE.
- 봉이 제안과 어긋나거나(예: 매수인데 고점 긴 윗꼬리·하락추세), 근거가 약하거나, 과도하거나, 의심스러우면 REJECT(보수적으로 반려).
